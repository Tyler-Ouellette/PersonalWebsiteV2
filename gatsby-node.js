...
exports.createPages = ({ graphql, actions }) => {
    ...
    return graphql(`
        {
            ...
            basepages: allMarkdownRemark(filter: { fileAbsolutePath: { regex: "/basepages/" } }) {
                edges {
                    node {
                        frontmatter {
                            template
    ...
    `).then(result => {
        ...
        const experiencePostsPerPage = result.data.limitPost.siteMetadata.experienceItemsPerPage;
        const numBlogPages = Math.ceil(experiencePosts.length / experiencePostsPerPage);
